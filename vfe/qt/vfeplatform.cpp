...
namespace vfePlatform
{
	...
	int vfeQtSession::RequestNewOutputPath(int CallCount, const string& Reason, const UCS2String& OldPath, UCS2String& NewPath)
    {
    ...
    // in the session.
	POV_LONG vfeQtSession::GetTimestamp(void) const
    {
...
// FIXME: add fallback using boost::xtime()
	...
	void vfeQtSession::WorkerThreadStartup()
    {
#ifdef _DEBUG
	...
	void vfeQtSession::WorkerThreadShutdown()
    {
    }

	void vfeQtSession::AppendErrorMessage (const string& msg)
    {
		QString qs = QString("fatal %1").arg(msg.c_str());
...
//    	std::stringstream ss;
//		ss << POVMS_UCS2toASCIIString(File) << "\" " << Line << " " << Col << " " << Msg;
		m_qtVfe->sendPovrayTextMessage(s_stream, qs);
	...
	bool vfeQtSession::StrCompare (const UCS2String& lhs, const UCS2String& rhs) const
    {
    ...
    // comparisons.
	bool vfeQtSession::TestPath (const Path& path, const Path& file, bool recursive) const
    {
	...
	bool vfeQtSession::TestAccessAllowed (const Path& file, bool isWrite) const
    {
